...
The following founder requirements, engineering proposals, and implementation pointers predate this formal standards review. They are preserved as historical pointers showing that several current controls grew from Velvet's own retrofit/OEM/safety theory work before they were mapped to named regulatory frameworks. Provenance strength and speaker/acceptance status are tracked separately in `decision_provenance.md`.
...
| **2025-04-28** | A contemporaneous engineering proposal used electrically separated chains such as UP Squared GPIO -> opto-isolator -> driver -> relay/motor rather than driving vehicle loads directly from compute; explicit founder acceptance of that exact chain remains to be sourced. | Fault containment, interface isolation, electrical integration and safety-related hardware design. |
